...
create_phoenix_transaction_cumulative <- function(phoenix_transaction_df){
    ...
    dplyr::group_by(award_number, period, program_area, fiscal_year, quarter) |>
    ...
    phoenix_transaction_disbursement_fy <- phoenix_transaction_df |>
        dplyr::select(award_number, fiscal_year, quarter, program_area, transaction_disbursement, period) |>
        dplyr::arrange(award_number, fiscal_year, quarter)  |>
        ...
        dplyr::ungroup() |>
        dplyr::select(-c(fiscal_year, quarter, transaction_disbursement))

    temp <- phoenix_transaction_df |>
        dplyr::left_join(phoenix_transaction_disbursement_fy, by = c("award_number", "period", "program_area")) |>
        dplyr::select(-c(fiscal_year, quarter))
